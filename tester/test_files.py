...
async def test_convert_image_to_jpg():
    """Upload a non-PNG image (JPEG) and convert it directly to JPG via
    ``target_format``.

    Sequence:
      1. Generate a small JPEG with Pillow.
      2. Upload it.
      3. Send ``convert_image`` with ``target_format: "jpg"`` and wait for the
         async ``convert_image_response``.
      4. Assert the response carries a new file whose name ends in ``.jpg``.
      5. Fetch the converted file and assert it starts with JPEG magic bytes.
    """
    print("\n[test] convert image to jpg (non-png source)")
    email, password = await register_and_login()
    ws = await open_ws()
    await send_and_wait(ws, "get_user_files",
                        {"email": email, "password": password},
